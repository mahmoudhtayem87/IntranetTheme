	...
	<#if portlet_display.getPortletConfigurationIconMenu()?? && portlet_display.getPortletToolbar()?? && portlet_display.isPortletDecorate() && portlet_display.isShowPortletTopper() && !portlet_display.isStateMax()>
		...
		<#if (portlet_configuration_icons?has_content || portlet_title_menus?has_content)>
			<header class="portlet-topper">
				<div class="portlet-title-default d-flex mx-2">
					<div class="portlet-name-text text-white">${portlet_display_name}</div>
					<button class="btn btn-sm btn-unstyled text-white mx-2" onClick='popup("${portlet_display.getURLConfiguration()}","${portlet_title}")'>
						<i class="fas fa-cog"></i>
					</button>
					<button class="btn btn-sm btn-unstyled text-white mx-2" onClick='popup("${portlet_display.getURLExportImport()}","${portlet_title}")'>
						<i class="fas fa-file-export"></i>
					</button>
				</div>

				<!--<#foreach portletTitleMenu in portlet_title_menus>
					<menu class="portlet-topper-toolbar 111" id="portlet-title-menu_${portlet_id}_${portletTitleMenu_index}" type="toolbar">
